...
# Helpers for type label resolution (added)
...
def _lookup_label(doc, iri: str, label_preds):
    # Try each label predicate; return the first label found
    for lp in label_preds:
        it, n = doc.search((iri, lp, None))
        for (_, _, lbl) in it:
            return str(lbl)
    return None
#End helpers
...
def createSentences(embed_list, doc, type_label_preds):
        ...
        for value_dict in value:
            for k, v in value_dict.items():
                if k == "label":
                    continue
                if k == "type":
                    lbl = _lookup_label(doc, v, type_label_preds)
                    if not lbl:
                        lbl = _last_segment(v)
                    if lbl and lbl not in type_labels:
                        type_labels.append(lbl)
                else:
                    sentence += f"; {k}: {v}"
        if type_labels:
            sentence += "; TYPES: " + " | ".join(type_labels)
...
def main(input_file: pathlib.Path, config_file: pathlib.Path, tsv_output, json_output, qdrant_url):
    ...
    try:
        ...
        # load type label predicates from config (with safe defaults)
        try:
            with open(config_file, 'r') as _cf:
                _cfg = yaml.safe_load(_cf)
            type_label_preds = _cfg.get("typeLabelPredicates", [
        ...
        sentences_to_embed_dict = createSentences(embed_list, doc, type_label_preds)
